...
5. We can use `useForm` hook to handle `title` and `description` but for file input we need to create a new function `handleFileChange` to `Upload.jsx`:
...
6. To submit the form, create `doUpload` function to `Upload.jsx`:
   ...
   - To upload the file to the file server using _FormData_, create a new hook `useFile` to `apiHooks.js`. Then add a new function `postFile`to `useFile` hook:
